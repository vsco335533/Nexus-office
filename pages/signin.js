...
export default function SignInPage() {
  const { signIn, signInWithGoogle, user } = useAppState();
  ...
  useEffect(() => {
    ...
    const checkInterval = setInterval(() => {
      if (win.google && win.google.accounts && googleDivRef.current) {
            ...
            callback: async (resp) => {
              if (resp.credential) {
                ...
                if (r.ok) {
                  ...
                  setErrorMsg(r.error || 'Google login failed');
                }
              }
            }
          });
          win.google.accounts.id.renderButton(googleDivRef.current, { 
            theme: 'filled_blue', 
            size: 'large',
            width: '380',
            text: 'signin_with',
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(res.error || 'Authentication key rejection. Check credentials.');
      }
    } catch (err) {
      setLoading(false);
      setErrorMsg('Failed to establish contact with secure server.');
    }
  };
      ...
      <div className="bg-slate-950/70 border border-red-500/15 p-8 md:p-10 rounded-3xl shadow-[0_0_50px_-12px_rgba(239,68,68,0.2)] flex flex-col max-w-md w-full relative z-10 animate-panel crt-screen backdrop-blur-xl transition-all duration-300">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-8 text-[9px] font-mono tracking-widest text-neutral-400">
          <div className="flex items-center gap-2">
            ...
            <span>SECURE WORKSPACE SYSTEM // IN-5001</span>
          </div>
          <span className="text-red-500 font-bold bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">ONLINE</span>
        </div>
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-neutral-500 text-[10px] mt-2 font-mono tracking-widest uppercase">AUTHORIZED PERSONNEL CONSOLE</p>
        </div>
        ...
        <form ref={formRef} onSubmit={onSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-[9px] font-bold text-neutral-400 uppercase tracking-widest mb-2 ml-1 font-mono">VAULT ACCESS CODE (PASSWORD)</label>
            <div className="relative">
              <input 
                name="password" 
                type="password" 
          ...
          <div className="flex flex-col gap-4 pt-3">
            ...
            >
              {loading ? (
                ...
                <span>INITIATE MAIN CONSOLE</span>
              )}
            </button>
            
            {/* Google OAuth Divider */}
            <div className="flex items-center justify-between text-[8px] font-mono text-neutral-600 uppercase tracking-widest py-1">
              <span className="w-1/4 h-[1px] bg-white/5"></span>
              <span>OR DELEGATE FEDERATION</span>
              <span className="w-1/4 h-[1px] bg-white/5"></span>
            </div>

            {/* Google OAuth Button Shell */}
            <div className="flex items-center justify-center w-full min-h-[44px] bg-black/30 border border-white/5 rounded-xl overflow-hidden hover:border-white/10 transition-colors" ref={googleDivRef}></div>
          </div>
        </form>
        ...
        <div className="mt-8 pt-5 border-t border-white/10 text-[9px] text-neutral-500 font-mono flex items-center justify-between">
          <Link href="/signup" className="text-red-500 hover:text-red-400 hover:underline tracking-wider transition-colors duration-150">[REQUEST VAULT KEY]</Link>
