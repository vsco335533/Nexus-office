// frontend/components/pages/TagsPage.jsx
'use client';
...
export default function TagsPage() {
  ...
  // Get unique tags
  const tags = Array.from(new Set(notes.filter(n => !n.trashed).flatMap(n => n.tags || [])));
  const filtered = tags.filter(t => t.toLowerCase().includes(searchQuery.toLowerCase()));

  const getNotesWithTag = (tag) => {
    return notes.filter(n => n.tags?.includes(tag) && !n.trashed);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-ash-800 rounded-xl p-6 border border-ash-200 dark:border-ash-700">
        <h1 className="text-3xl font-bold text-ash-900 dark:text-white">Tags</h1>
        <p className="text-ash-600 dark:text-ash-400 mt-1">Organize notes with tags</p>
        <p className="text-sm text-ash-500 dark:text-ash-400 mt-2">{filtered.length} tag{filtered.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-ash-800 rounded-xl p-12 border border-ash-200 dark:border-ash-700 text-center">
          <Icons.Tag className="w-16 h-16 text-ash-300 dark:text-ash-600 mx-auto mb-4" />
          <p className="text-ash-500 dark:text-ash-400">No tags found</p>
        </div>
      ) : (
        <div className="space-y-6">
          {filtered.map((tag) => {
            const tagNotes = getNotesWithTag(tag);
            return (
              <div key={tag} className="bg-white dark:bg-ash-800 rounded-xl border border-ash-200 dark:border-ash-700 overflow-hidden">
                {/* Tag Header */}
                <div className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 p-6 border-b border-ash-200 dark:border-ash-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-ash-900 dark:text-white">#{tag}</h3>
                      <p className="text-sm text-ash-600 dark:text-ash-400">{tagNotes.length} note{tagNotes.length !== 1 ? 's' : ''}</p>
                    </div>
                  </div>
                </div>
